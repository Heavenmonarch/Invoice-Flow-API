...
from app.schemas.user import StaffInvite, UserUpdate, UserOut
...
@router.post("/invite-users", response_model=UserOut, status_code=201)
async def invite_staff(
    payload: StaffInvite,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_admin),
):
    return await UserService.invite_staff(
        payload, current_user.organization_id, current_user, db
    )
...
async def update_user(
    ...
    return await UserService.update_user(
        user_id, payload, current_user.organization_id, current_user, db
    )
...
async def deactivate_user(
    ...
    return await UserService.deactivate_user(
        user_id, current_user.organization_id, current_user, db
    )